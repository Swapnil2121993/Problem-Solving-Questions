...
// Gotchas
// Does your function work with negative numbers? If arrayOfInts is [-10, -10, 1, 3, 2][−10,−10,1,3,2] 
//we should return 300300 (which we get by taking -10 * -10 * 3−10∗−10∗3).
...
// Sorting the array would let us grab the highest numbers quickly, so it might be a good first step. Sorting takes O(n\lg{n})O(nlgn) time. That's better than the O(n^3) 
//time our brute force approach required, but we can still do better.

// Since we know we must spend at least O(n) time, let's see if we can solve it in exactly O(n)O(n) time.
